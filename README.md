...
Once Flask is running, run the comprehensive test suite:

```bash
# In a new terminal, from backend directory
python verify_setup.py                  # Verify system is ready
python tests/run_pipeline_tests.py      # Run full pipeline tests
```

**What gets tested:**
- ✓ 5 realistic internship scenarios (legitimate, scam, startup, suspicious, borderline)
- ✓ All 6 validation pipelines per test case
- ✓ Complete data flow through all stages
- ✓ Detailed JSON report with results

📖 **Testing Documentation**: See [backend/TESTING_INDEX.md](backend/TESTING_INDEX.md) for complete testing guides
...
│   ├── TESTING_INDEX.md              # 🧪 Test documentation hub
│   ├── COMPLETE_TESTING_SUMMARY.md   # 🧪 Comprehensive test summary
│   ├── QUICK_TESTING_GUIDE.md        # 🧪 5-minute test quickstart
│   ├── PIPELINE_TESTING_GUIDE.md     # 🧪 Detailed testing guide
│   ├── PIPELINE_ARCHITECTURE.md      # 🧪 Visual pipeline diagrams
│   ├── API_SETUP_GUIDE.md            # API configuration guide
│   │
│   ├── tests/                        # 🧪 Test suite (NEW)
│   │   ├── test_datasets.py         # 5 realistic test cases
│   │   ├── run_pipeline_tests.py    # Automated test runner
...
│   │   ├── credibility_engine.py    # Main scoring logic (with dataset validation)
│   │   ├── sentiment_analyzer.py    # DistilBERT sentiment
│   │   ├── company_verifier.py      # Company legitimacy check (with Google CSE API)
│   │   ├── dataset_validator.py     # HuggingFace/Kaggle dataset validation (NEW)
...
│   │
│   ├── data/                         # Local data files (NEW)
│   │   ├── legitimate_companies.json # 23 verified companies
│   │   ├── scam_companies.json      # Known scam companies
│   │   └── scam_patterns.json       # Scam pattern definitions
│   │
